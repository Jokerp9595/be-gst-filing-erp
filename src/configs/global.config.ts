...
export class Global {
    ...
    static async token(params: any, isRefreshToken: boolean = false) {
        return new Promise((resolve, reject) => {
            try {
                const tokenData = {
                    ...
                    exp: Math.floor(new Date().getTime() / 1000) + (isRefreshToken ? configuration.refreshTokenExpiry : configuration.tokenExpiry)
                };
    ...
    forgotPasswordToken(params: any) {
        return new Promise((resolve, reject) => {
            try {
                const tokenData = {
                    id: params.id,
                    ipAddress: params.ipAddress,
                    exp: Math.floor(new Date().getTime() / 1000) + configuration.forgotPasswordTokenExpiry
                };
                const token = jwt.sign(tokenData, configuration.securityToken);
                resolve(token);
            } catch (err: any) {
    ...
    keyExists(key: string, search: any) {
                ...
                if (FieldHelperService.undefinedAndNullCheck(decodedForVerification)) {
                    if (authenticate.countProperties(decodedForVerification) === 5) {
                        if (authenticate.keyExists("id", decodedForVerification) &&
                            ...
                            return res.status(401).json({ status: 0, message: response["105"] });
                        }
                    }
                    else {
                        return res.status(401).json({ status: 0, message: response["105"] });
    ...
    static async isResetPasswordAuthorized(req: any, res: any, next: any) {
        try {
            let token = req.headers.authorization;
            if (!token) {
                return res.status(401).json({ status: 0, message: response["103"] });
                ...
                if (FieldHelperService.undefinedAndNullCheck(decodedForVerification)) {
                    if (authenticate.countProperties(decodedForVerification) === 4) {
                        if (authenticate.keyExists("id", decodedForVerification) &&
                            ...
                            const tokenExpire = await authenticate.checkExpiration(decodedForVerification);
                            if (!tokenExpire) {
                                return res.status(200).json({ status: 0, message: response["111"] });
                            }
                            else {
                                next();
                                return true;
                            ...
                            return res.status(200).json({ status: 0, message: response["105"] });
                        }
                    }
                    else {
                        return res.status(200).json({ status: 0, message: response["105"] });
                ...
                return res.status(200).json({ status: 0, message: response["103"] });
            }
        } catch (err: any) {
            applicationLogger.error(`Global.js isResetPasswordAuthorized`, { error: err.toString() });
    ...
    static async getTokenValue(req: any, param: any) {
        try {
            ...
            if (decodedForVerification && typeof decodedForVerification === 'object' && param in decodedForVerification) {
                return (decodedForVerification as any)[param];
            }
            return "";
        } catch (err: any) {
    ...
    static async encrypt(value: string | any[]) {
        let encryptString = '';
    ...
    static async decrypt(value: string | any[]) {
        let decryptString = '';
        const stringLength = value.length;
